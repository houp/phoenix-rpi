...
gate (proves UI VM + renderer + present all work end-to-end). REMAINING before org push: regression-check the shared
libSDL2.a fix against quakespasm-sdl/yQuake2 (should be unaffected — the fix only adds correct window events), THEN
push ports e498158 + snapshot a manifest. Artifacts: artifacts/hdmi/20260810-213936-q3render-tick.png.
