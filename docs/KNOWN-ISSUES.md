...
Open items in the Phoenix-RTOS Raspberry Pi 4 port, as of 2026-08-05. This is
...
| #70 | Dillo HTTPS/TLS. | **Build-capable (2026-08-05):** Dillo 3.2.0 now builds with TLS via **mbedTLS** (`build-dillo.sh --enable-tls --disable-openssl`); the binary links clean with the TLS backend actively wired. Live HTTPS browsing additionally needs a CA-cert bundle + Pi internet access (host NAT) — tracked as E2/E3. Plain HTTP works today. |
