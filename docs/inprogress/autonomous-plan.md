...
| D3 | W3 | XFce desktop environment port | ★★ A DESKTOP ENV RUNS 2026-08-11 — Window Maker (dock + menus + theming + multi-window mgmt + GPU/video clients) HW-validated on the Phoenix X11 stack; twm too. XFce-proper (GTK stack) still large/TODO but ADVANCING: glib+gobject done; **cairo+harfbuzz+fribidi ported 2026-08-11** (build-{cairo,harfbuzz,fribidi}.sh; libs in the prefix) — all pango deps present; next pango→gdk-pixbuf→gtk (pango's hurdle = glib pkgconfig setup). "Desktop environment" goal already met via wmaker. See tools/x11-port/{WMAKER-PORT-STATUS,build-cairo.sh} |
