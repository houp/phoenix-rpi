# Integration State: 2026-09-18-w38-audio-armguard-gated
...
| --- | --- | --- | --- |
| _build | main | 7c2deb633 (dirty(1)) | https://github.com/houp/phoenix-rpi.git |
...
| phoenix-rtos-devices | master | 2e9286d (clean) | https://github.com/phoenix-rtos/phoenix-rtos-devices.git |
...
_build	7c2deb633c582f78f2004738c585ae38f69e3bf7	main
...
phoenix-rtos-devices	2e9286de3fad62b0fc8febb9ba8406fecbea09b9	master
...
## ⚠ Which devices SHA the gate actually ran on

The six-app gate this manifest is named for (`mashgate`, 2026-09-18 18:06 — 6/6, 0 faults, frames on
all five GPU apps, torches present, audio streaming on arm 1 with `self-test fed 8960 samples` on all
six boots) ran on `phoenix-rtos-devices` **`d729d77`**.

The SHA recorded above, `2e9286d`, is one commit later: it adds the **diagnostic
`RPI4AUDIO_ARMTRIALS` ioctl**, which is additive to the ABI and unreachable unless a client calls it,
so the boot path the gate exercised is byte-for-byte the same code. It is **not** covered by that
gate; it was boot-checked separately. Restore `d729d77` if you want exactly the gated tree.
