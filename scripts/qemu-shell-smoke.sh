...
case "$target" in
	generic)
		...
		;;
	rpi4b)
		limactl shell -y "$vm" -- /bin/bash -lc 'cd /home/witoldbolt.guest/phoenix-buildroots/phoenix-rtos-project-copy && expect -c "set timeout 35; log_user 1; spawn /home/witoldbolt.guest/tools/qemu-10.2.2/bin/qemu-system-aarch64 -M raspi4b -cpu cortex-a72 -smp 4 -m 2G -nographic -monitor none -kernel _boot/aarch64a72-generic-rpi4b/plo.elf -device loader,file=_boot/aarch64a72-generic-rpi4b/rpi4b/loader.disk,addr=0x48000000,force-raw=on; expect \"(psh)%\"; sleep 1; send \"help\r\"; expect \"Available commands:\"; expect \"(psh)%\"; send \003; expect eof" > /tmp/pi4-shell-smoke.log 2>&1 && grep -En "Available commands:|\(psh\)%|help" /tmp/pi4-shell-smoke.log | tail -n 20 && printf "Log: %s\n" /tmp/pi4-shell-smoke.log'
		;;
	*)
		usage
		exit 2
