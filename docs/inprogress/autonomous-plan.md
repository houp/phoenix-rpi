...
| T-DYNLINK | W3 | **Dynamic linking / shared-library support (general system feature)** — owner 2026-08-10 | TODO (feasibility first) | Owner: assess feasibility, then design+implement+test as a GENERAL feature (not Pi4-only). Today everything is static single-ELF (no dlopen) — this blocks true GLX/DRI, runtime plugins, QVM-less mods, smaller images. Scope: ELF loader (kernel proc load path), libphoenix (dlopen/dlsym + PLT/GOT), syspage/exec, toolchain (-fPIC/-shared, .so search). Start with a feasibility+design doc before touching the kernel. See [[feedback_owner_directive_2026_08_10]] |
| T-WIFI-BT | W3 | **WiFi + BT as first-class Phoenix citizens** — owner 2026-08-10 | TODO (design first) | Both BCM43455 radios work as tools/ probes ([[project_wifi_fw_exec_gate_91]], [[project_bluetooth_bringup]]). Owner wants real integration: config files, CLI utils, multi-BT-device support; maybe separate subprojects (like phoenix-rtos-usb) or additions to -devices/-tools. Start with a subproject-layout + config + CLI design doc, then implement tractable pieces. See [[feedback_owner_directive_2026_08_10]] |
