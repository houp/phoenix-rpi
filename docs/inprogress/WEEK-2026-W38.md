...
⚠ **While the card is in, netboot is unavailable** — it is an either/or with gates, soaks and reel
captures. `scripts/test-sd-boot.sh` brings dnsmasq down, runs the pair, and restores the server on every
exit path.
...
- 🐞 **`q2-sdl-openaudio-hang` (new, filed).** In gate 2 Quake II rendered **zero frames** — while the
