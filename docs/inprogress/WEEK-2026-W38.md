...
⚠ **"Add a timeout" is NOT the fix:** `nfs_makeContext()` already calls `nfs_set_timeout(nfs, 5000)`
and is the only context factory, so that timeout demonstrably does not bound `nfs_renew()`.
⏭ **Next:** why the 5 s timeout does not apply to this call (libnfs sync path), and whether the
renewal should run off the message loop entirely so a stalled lease RPC cannot take the filesystem
down with it. A timeout on the fs-path IPC remains worth having as containment either way.
