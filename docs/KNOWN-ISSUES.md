...
| C2 | **A stack-overflow crash prints two nested kernel dumps instead of one useful one.** `exceptions_dumpUserStack()` dereferences the user stack; in an overflow the page holding `sp` is unmapped by construction, so the first read takes a Data Abort at EL1 (`far == sp - 128`). ↩ *Corrected 2026-09-23 — this row used to say a stack-exhaustion DoS **takes down the kernel**. It does not.* Measured on `tools/stack-bomb` at `-O0`: 3 boots, 5 bomb runs, and **3 bombs in a single boot** — the process dies, psh keeps working, follow-up commands all run. The site is the dumper, not `hal_cpuPushSignal`, and the fault is a **read** (`esr=0x96000007`) where 2026-08-22 recorded a write. | **Guarded (kernel `HEAD`)** with `at s1e1r` + `PAR_EL1.F` — lock-free, since this runs in exception context. ⚠ Residual, separate and unmeasured: `hal_cpuPushSignal` still writes the signal frame to the user stack unconditionally (`hal/aarch64/cpu.c`), it is simply not what the reproducer reaches. |
