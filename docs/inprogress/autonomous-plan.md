...
| E1 | W2 | Dillo HTTPS support | WIP | 2026-08-05: openssl-1.1.1a + mbedtls-2.28.0 ARE built (sysroot libs) — the build-dillo.sh comment "no TLS lib yet" is STALE. Dillo 3.2.0 currently built `--disable-tls` (#70). Subagent enabling TLS (prefer mbedTLS = Apache/GPL-clean; else openssl) + resolving Dillo's final link (the real risk). Deliverable = HTTPS-CAPABLE build (configure+link+TLS symbols); end-to-end HTTPS browsing needs Pi internet (E2, deferred). See #70 |
