...
export PKG_CONFIG_LIBDIR="$PREFIX/lib/pkgconfig:${ROOT}/tools/ports/src/glib-2.56.4"  # glib .pc (--with-glib=yes needs it; host-isolated)
...
CC="${TC}gcc --sysroot=$SYSROOT" CXX="${TC}g++ --sysroot=$SYSROOT" AR="${TC}ar" RANLIB="${TC}ranlib" \
  ...
  ./configure --host=aarch64-phoenix --prefix="$PREFIX" --disable-shared --enable-static \
    --with-freetype=yes --with-glib=yes --with-gobject=no --with-icu=no --with-cairo=no
make -C src -j"$(nproc)"
make -C src install
