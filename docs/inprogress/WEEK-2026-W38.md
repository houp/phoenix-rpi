...
`mark=5` is set immediately **before `nfs_service()`** and 6 (immediately after) is never set.
⇒ **The thread is blocked inside `nfs_service()` → `rpc_service()`, not in the wait loop** — which is
why an overall deadline in the loop could not fire. `poll()` is not the culprit (mark 3→4 passed).
⏭ **Next:** finer marks inside `rpc_service` (`recv`/`readv`/`writev`/reconnect) to name the exact
syscall. Prime suspect: a socket call that ignores `O_NONBLOCK` and never returns once the peer is
gone — i.e. a **Phoenix lwip/libphoenix** defect, not a libnfs one.
ⓘ This wedge showed `phase=2` (open path), the previous one `phase=20` (renew). Same blocking point,
different caller — consistent with "whichever op is in flight when the server drops us".
ⓘ **Scope:** `nfs-fs` exists only on the **netboot / nfsroot** variant. The showcase videos and the
gated demo image are **SD-boot** (ext2 root, no NFS), so this failure mode cannot occur there. Every
`premain-hang` observation to date came from a netboot cycle. To be confirmed with an SD bench.
