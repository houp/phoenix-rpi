...
| C3 | **Unmounting a USB stick during I/O loses the data.** A 200 MiB write, unmounted 2 s in: `umount` returns **rc=0** while the writer gets `Invalid argument` after 5 MiB. Two distinct defects behind it — `umass_poolthr` can `free()` the filesystem context while another pool thread is inside `libext2_handler`, and `_umass_partUnmount` calls `portDestroy()` after waiting on a flag that is cleared as soon as the unmount is *queued*. | **Open, found 2026-09-22, not fixed.** Baseline measured (`umountrace-A`). Two fix attempts on branch `agent/umass-umount-drain` both failed the A/B: the second turned the failure from "fails fast, loses data" into "the writer hangs forever". Branch is WIP, **not merged**; master is unaffected. |
