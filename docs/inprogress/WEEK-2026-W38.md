...
⏭ **Open question raised today:** `fbcon: ok` has not appeared in any netboot log since 2026-08-14 —
neither it nor its `fbcon init failed` sibling — so `uart-summary.sh`'s `fbcon up` row reads ✗ on
boots whose HDMI is demonstrably fine (X scored 89.8% non-black). Either the console fbcon path stopped
running on netboot, or the stage check is stale. Decide which before trusting that row again.
